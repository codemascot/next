...
Change  the =next/git:*git-projects-roots*=  list to  define where  to
look for existing git repositories. By default, the git-clone commands
looks into =~/projects= and =~/work=.

#+begin_src lisp
(setf next/git:*git-projects-roots* '("~/my/directory"))
#+end_src
